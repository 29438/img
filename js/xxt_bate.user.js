...
// @namespace           666
// @version             1.4.1
// @description         实现功能：开放自定义设置、新版考试、视频倍速\秒过、文档秒过、答题、收录答案、作业、收录作业答案、读书秒过。
// @author              Ne-21
// @match               *://*.chaoxing.com/*
// @match               *://*.edu.cn/*
// @match               *://*.nbdlib.cn/*
...
function showBox() {
    if ($('#ne-21notice')[0] == undefined) {
        ...
        <h3 style="text-align: center;">超星学习小助手<span style="color:red;">(K键隐藏面板)</span></h3>
            ...
            success: function (res) {
                try {
                    ...
                    if (setting.rate == 0) {
                        ...
                        logger('已开启视频倍速，当前倍速：' + setting.rate + '，2倍速以上可能会导致进度重置、挂科等问题。', 'red')
...
function startDoPhoneCyWork(index, doms, phoneWeb) {
    ...
    getElement($(doms[index]).contents()[0], 'iframe').then(element => {
        ...
        if (workIframe.length == 0) {
            setTimeout(() => { startDoPhoneCyWork(index, doms) }, 4000)
        }
        ...
        if (workStatus.indexOf("已完成") != -1) {
            ...
            setTimeout(() => { upLoadWork(index, doms, workIframe) }, 4000)
...
function startDoCyWork(index, doms) {
    ...
    getElement($(doms[index]).contents()[0], 'iframe').then(element => {
        let workIframe = element
        if (workIframe.length == 0) {
            setTimeout(() => { startDoCyWork(index, doms) }, 4000)
        }
        let workStatus = $(workIframe).contents().find('.CeYan .ZyTop h3 span:nth-child(1)').text().trim()
        if (workStatus.indexOf("已完成") != -1) {
            logger('测验：' + (index + 1) + ',检测到此测验已完成,准备收录答案。', 'green')
            ...
            logger('测验：' + (index + 1) + ',准备处理此测验...', 'purple')
            setTimeout(() => { doWork(index, doms, workIframe) }, 4000)
        } else if (workStatus.indexOf('待批阅') != -1) {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',测验待批阅,跳过', 'red')
            setTimeout(() => { startDoCyWork(index + 1, doms) }, 4000)
        } else {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',未知状态,跳过', 'red')
